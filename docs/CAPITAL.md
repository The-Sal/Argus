...
def track_spread(symbol, data):
    spread = data.ask - data.bid
    if symbol not in spreads:
        spreads[symbol] = {'min': spread, 'max': spread, 'count': 0}

    spreads[symbol]['min'] = min(spreads[symbol]['min'], spread)
    spreads[symbol]['max'] = max(spreads[symbol]['max'], spread)
    spreads[symbol]['count'] += 1

client.stream_symbols(['GBPUSD', 'EURUSD', 'USDJPY'], callback=track_spread)

time.sleep(60)  # Collect 1 minute of data

for symbol, stats in spreads.items():
    print(f"{symbol}: min=${stats['min']:.5f}, max=${stats['max']:.5f}, ticks={stats['count']}")

client.close()
```

### Example 3: Bulk Symbol Loading

```bash
# Create symbols file
cat > /tmp/forex_majors.txt <<EOF
EURUSD
GBPUSD
USDJPY
AUDUSD
...
client.send_dict({
    'action': 'resolve/stream/batch/file',
    'file': '/tmp/forex_majors.txt'
})

def log_tick(symbol, data):
    print(f"{symbol}: {data.bid}")

client.symbol_callback = log_tick
...
# Don't provide callback - just use state tracking
client.stream_symbols(['BTCUSD', 'ETHUSD'])

time.sleep(5)  # Let some data arrive

# Access latest state
for symbol, data in client.states.items():
    print(f"{symbol}: bid={data.bid}, ask={data.ask}, timestamp={data.timestamp}")

client.close()
```

### Example 5: Unsubscribe Management
...
# Resolve BTCUSD to get EPIC
client.send_dict({'action': 'resolve_symbol', 'symbol': 'BTCUSD'})
time.sleep(1)  # Wait for response (check logs)

# Manually stream EPIC
client.send_dict({'action': 'stream_epic', 'epic': 'BTCUSD'})
time.sleep(10)

# Unsubscribe
client.unsubscribe('BTCUSD')
print("Unsubscribed from BTCUSD")
